...
RETURNS SETOF pracownik AS $$
BEGIN
	RETURN QUERY SELECT * FROM pracownik where imie LIKE ('%' || vImie || '%')
										 and nazwisko LIKE ('%' || vNazwisko || '%')
										 and zawod LIKE ('%' || vZawod || '%')
										 and vMinPlaca <= placa and placa <= vMaxPlaca;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS SETOF wagon AS $$
BEGIN
	RETURN QUERY SELECT * FROM wagon where vDataMin <= databadaniatechnicznego and databadaniatechnicznego <= vDataMax
									 and vMiejscMin <= liczbamiejsc and liczbamiejsc <= vMiejscMax;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS SETOF znizka AS $$
BEGIN
	RETURN QUERY SELECT * FROM znizka where nazwaznizki LIKE ('%' || vNazwa || '%')
									  and dokumentpotwierdzajacy LIKE ('%' || vDokument || '%')
									  and vProcentMin <= procentznizki and procentznizki <= vProcentMax;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS SETOF lokomotywa AS $$
BEGIN
	RETURN QUERY SELECT * FROM lokomotywa where vDataMin <= databadaniatechnicznego and databadaniatechnicznego <= vDataMax
...
RETURNS SETOF pociag AS $$
BEGIN
	RETURN QUERY SELECT * FROM pociag where vIdLokomotywyMin <= idlokomotywy and idlokomotywy <= vIdLokomotywyMax
									  and nazwa LIKE ('%' || vNazwa || '%');
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION wagonwpociaguFilter(IN vIdWagonuMin INTEGER DEFAULT 0,
											   ...
											   IN vIdPociaguMin INTEGER DEFAULT 0,
									   		   IN vIdPociaguMax INTEGER DEFAULT 99999)
RETURNS SETOF wagonwpociagu AS $$
BEGIN
	RETURN QUERY SELECT * FROM wagonwpociagu where vIdWagonuMin <= idwagonu and idwagonu <= vIdWagonuMax
									  		 and vIdPociaguMin <= idpociagu and idpociagu <= vIdPociaguMax;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS INTEGER AS $$
BEGIN
	if((select count(*) from wagonwpociagu where idpociagu = vIdPociagu and idwagonu = vIdWagonu) != 1) then return 0; end if;
...
BEGIN
	if((select count(*) from wagonwpociagu where idpociagu = vIdPociagu 
										   and idwagonu = vIdWagonu) != 1) then return 0; end if;
...
BEGIN
	if((select count(*) from wagonwpociagu where idpociagu = vIdPociagu 
										   and idwagonu = vIdWagonu) != 0) then return 0; end if;
	if(vIdWagonu < 0 or vIdPociagu < 0 or vNrWagonu < 0) then return -1; end if;
	if((select count(*) from wagonwpociagu where idpociagu = vIdPociagu 
										   and numerwagonu = vNrWagonu)!= 0) then return -2; end if;
	if((select count(*) from wagon where vIdWagonu=wagon.id) != 1) then return -3; end if;
...
RETURNS SETOF stacja AS $$
BEGIN
	RETURN QUERY SELECT * FROM stacja where adres LIKE ('%' || vAdres || '%')
									  and nazwa LIKE ('%' || vNazwa || '%');
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS SETOF przystanek AS $$
BEGIN
	RETURN QUERY SELECT * FROM przystanek where vNrPrzystankuMin <= numerprzystanku and numerprzystanku <= vNrPrzystankuMax
									  		 and vNrLiniiMin <= idlinii and idlinii <= vNrLiniiMax
											 and nazwastacji LIKE ('%' || vNazwaStacji || '%') order by idlinii;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
RETURNS INTEGER AS $$
BEGIN
	if((select count(*) from przystanek where nazwastacji = vNazwaStacji and idlinii = vNrLinii) != 1) then return 0; end if;
...
BEGIN
	if((select count(*) from przystanek where nazwastacji = vNazwaStacji 
										   and idlinii = vNrLinii) != 1) then return 0; end if; -- na linii nie ma tego przystanku
	if(vNrPrzystanku < 0 or length(vNazwaStacji) < 1 or vNrLinii < 0) then return -1; end if;
	if((select count(*) from przystanek where idlinii = vNrLinii 
										   and numerprzystanku = vNrPrzystanku)!= 0) then return -2; end if; -- na linii jest już ten numer zajęty
	update przystanek
	SET numerprzystanku = vNrPrzystanku
		where nazwastacji = vNazwaStacji and idlinii = vNrLinii;
	return 1;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
...
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION przejazdFilter(IN vIdMin INTEGER DEFAULT 0,
										  IN vIdMax INTEGER DEFAULT 0,
										  IN vOdjazdOd TIMESTAMP DEFAULT '23-12-2022',
										  ...
										  IN vIdKonduktoraOd INTEGER DEFAULT 0,
										  IN vIdKonduktoraDo INTEGER DEFAULT 9999,
										  IN vIdMaszynistyOd INTEGER DEFAULT 0,
										  IN vIdMaszynistyDo INTEGER DEFAULT 9999,
										  IN vIdLiniiPrzejazduOd INTEGER DEFAULT 0,
										  IN vIdLiniiPrzejazduDo INTEGER DEFAULT 9999,
										  IN vIdPociaguOd INTEGER DEFAULT 0,
										  IN vIdPociaguDo INTEGER DEFAULT 9999)
RETURNS SETOF przejazd AS $$
BEGIN
	RETURN QUERY SELECT * FROM przejazd where vIdMin <= id and id <= vIdMax
									  	  and vOdjazdOd <= dataodjazdu and dataodjazdu <= vOdjazdDo
										  and vPrzyjazdOd <= dataprzyjazdu and dataprzyjazdu <= vPrzyjazdDo
										  and vIdKonduktoraOd <= idkonduktora and idkonduktora <= vIdKonduktoraDo
										  and vIdMaszynistyOd <= idmaszynisty and idmaszynisty <= vIdMaszynistyDo
										  and vIdLiniiPrzejazduOd <= idliniiprzejazdu and idliniiprzejazdu <= vIdLiniiPrzejazduDo
										  and vIdPociaguOd <= idpociagu and idpociagu <= vIdPociaguDo;
...
RETURNS INTEGER AS $$
BEGIN
	if((select count(*) from pracownik where pracownik.id = vIdKonduktora 
										 and pracownik.zawod = 'Konduktor') != 1) then return 0; end if;
	if((select count(*) from pracownik where pracownik.id = vIdMaszynisty 
										 and pracownik.zawod = 'Maszynista') != 1) then return -1; end if;
...
RETURNS SETOF rezerwacja AS $$
BEGIN
	RETURN QUERY SELECT * FROM rezerwacja where imie LIKE ('%' || vImie || '%')
									    and znizka LIKE ('%' || vZnizka || '%')
										and nazwisko LIKE ('%' || vNazwisko || '%')
										and vIdPrzejazduMin <= idprzejazdu and idprzejazdu <= vIdPrzejazduMax;
END
$$ LANGUAGE plpgsql;
---------------------------------------------------------------------------------
